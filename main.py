...
def parse_args():
    ...
    # 1
    parser.add_argument('--name', type=str, default='inverter1')
    # 2
    parser.add_argument('--config_file', type=str, default='./Config/inverter1.yaml',
    ...
    parser.add_argument('--missing_ratio', type=float,
                        default=0., help='Ratio of Missing Values.')
...
def main():
    ...
    if args.train:
        #samples = dataset.scaler.inverse_transform(samples.reshape(-1, samples.shape[-1])).reshape(samples.shape) ##Added Separately
        print('训练')
        trainer.train()
    elif args.sample == 1 and args.mode in ['infill', 'predict']:
        print('sample == 1 and args.mode in [infill, predict]')
        ...
        if dataset.auto_norm:
            ...
            np.save(os.path.join(args.save_dir,
                    f'ddpm_{args.mode}_{args.name}.npy'), samples)
    else:
        print('生成数据')
        trainer.load(args.milestone)
        dataset = dataloader_info['dataset']
        # 生成数据大小 size_every
        samples = trainer.sample(num=len(dataset), size_every=17832, shape=[
                                 dataset.window, dataset.var_num])
        if dataset.auto_norm:
            samples = unnormalize_to_zero_to_one(samples)
            # 将下面一行注释掉得到的是归一化的结果
            samples = dataset.scaler.inverse_transform(
                samples.reshape(-1, samples.shape[-1])).reshape(samples.shape)
            np.save(os.path.join(args.save_dir,
                    f'ddpm_fake_{args.name}.npy'), samples)
